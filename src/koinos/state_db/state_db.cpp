...
public:
  ...
  void open( const std::optional< std::filesystem::path >& p,
             ...
             const unique_lock_ptr& lock );
  void open( std::shared_ptr< backends::abstract_backend > backend,
             genesis_init_function init,
             state_node_comparator_function comp,
             const unique_lock_ptr& lock );
  void open_lockless( const std::optional< std::filesystem::path >& p,
                      genesis_init_function init,
                      state_node_comparator_function comp );
  void open_lockless( std::shared_ptr< backends::abstract_backend > backend,
                      genesis_init_function init,
                      state_node_comparator_function comp );
...
void database_impl::open( std::shared_ptr< backends::abstract_backend > backend,
                          genesis_init_function init,
                          state_node_comparator_function comp,
                          const unique_lock_ptr& lock )
{
  ...
  std::lock_guard< std::timed_mutex > index_lock( _index_mutex );
  std::unique_lock< std::shared_mutex > fork_heads_lock( _fork_heads_mutex );
  open_lockless( std::move( backend ), init, comp );
}
...
void database_impl::open_lockless( std::shared_ptr< backends::abstract_backend > backend,
                                   genesis_init_function init,
                                   state_node_comparator_function comp )
{
  auto root           = std::make_shared< state_node >();
  root->_impl->_state = std::make_shared< state_delta >( std::move( backend ) );
  _init_func          = init;
  _comp               = comp;
                                                    ...
                                                    const shared_lock_ptr& lock ) const
{
  ...
  KOINOS_ASSERT( node_itr != _index.end(),
                 internal_error,
                 "could not find state node associated with linked state_delta ${id}",
                 ( "id", delta->id() ) );
                                           ...
                                           const std::unordered_set< state_node_id >& whitelist )
{
  ...
  {
    KOINOS_ASSERT( remove_queue[ i ] != head_id,
                   cannot_discard,
                   "cannot discard a node that would result in discarding of head" );
...
void database::open( std::shared_ptr< backends::abstract_backend > backend,
                     genesis_init_function init,
                     state_node_comparator_function comp,
                     const unique_lock_ptr& lock )
{
  impl->open( std::move( backend ), init, comp, lock ? lock : get_unique_lock() );
}
